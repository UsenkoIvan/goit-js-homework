...
function inputCountryHandler(evt) {
  ...
  fetchCountry.fetchCountries(inputValue).then(country => {
    // console.log(country);
    ...
    if (country.length >= 10) {
      ...
      refs.countryInfo.insertAdjacentHTML('beforeend', createTemplateToHTML(markup))
    }
...
function createListCountry(drowListItems) {
  clearHTML()
  const markup = drowListItems.map(({
    name
  }) => {
